...
If you are completely new to MariaDB and relational databases, you may want to start with [A MariaDB Primer](/docs/server/server-usage/basics/mariadb-usage-guide-1). Also, ensure you understand the connection parameters discussed in the [Connection Parameters Guide](/docs/server/mariadb-quickstart-guides/mariadb-connecting-guide).
...
If the server isn't running, [start it using the method applicable to your environment](/docs/server/server-management/starting-and-stopping-mariadb/starting-and-stopping-mariadb-automatically).
...
* **Socket File Mismatch (Unix):** The socket file path might be non-standard or inconsistent between server and client configurations.
  * Check your [configuration file](/docs/server/server-management/install-and-upgrade-mariadb/configuring-mariadb/configuring-mariadb-with-option-files#server-option-groups). Ensure the `socket` option has the identical value for both server and client.
  * To find the running Unix socket file, try this command:
    ...
    ```
* See also: [Troubleshooting Installation Issues](/docs/server/server-management/install-and-upgrade-mariadb/installing-mariadb/troubleshooting-installation-issues).
...
**Solution:** See [Configuring MariaDB for Remote Client Access](/docs/server/server-usage/connecting/mariadb-remote-connection-guide-1) for detailed instructions on how to enable remote connections by adjusting the `bind-address` server variable and ensuring user accounts are configured correctly for remote hosts.
...
* **Unix Socket Authentication:** On Unix-like systems, the `unix_socket` authentication plugin is enabled by default for local connections via the Unix socket file. This plugin uses operating system user credentials.
  * See the [`unix_socket` authentication plugin documentation](/docs/server/reference/plugins/authentication-plugins/authentication-plugin-unix-socket) for connection instructions and how to switch to password-based authentication if needed.
  * For an overview of authentication changes in MariaDB 10.4, see [Authentication from MariaDB 10.4](/docs/server/security/user-account-management/authentication-from-mariadb-10-4).
* **Incorrect Username/Host Combination:** Authentication is specific to a `username@host` combination. For example, `'user1'@'localhost'` is distinct from `'user1'@'166.78.144.191'`. Ensure the user account exists for the host from which you are connecting.
  * See [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant) for details on granting permissions.
...
* **Solution:** Grant the necessary `FILE` privilege to the user. See the [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant) article.
...
* **Solution:** Grant the required privileges (e.g., `SELECT`, `INSERT`, etc.) on the specific database to the user. See [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant).
...
* **Troubleshooting:**
  * Check the values in any option files read by your client. See [Configuring MariaDB with Option Files](/docs/server/server-management/install-and-upgrade-mariadb/configuring-mariadb/configuring-mariadb-with-option-files) and the documentation for the specific client you are using (listed under [Clients and Utilities](/docs/server/clients-and-utilities)).
...
`mariadb --user myadmin --password` (specify *your\_password* when prompted)
{% endhint %}
...
* [CREATE USER](/docs/server/reference/sql-statements/account-management-sql-statements/create-user)
* [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant)
* [Authentication](/docs/server/security/user-account-management/authentication-from-mariadb-10-4)
* [Authentication from MariaDB 10 4](https://www.youtube.com/watch?v=aWFG4uLbimM) (video • 20 minutes • 2020)
* [Error 1698: Access denied for user](/docs/server/reference/error-codes/mariadb-error-codes-1600-to-1699/e1698)
