...
While [this page](/docs/server/server-usage/backup-and-restore/mariadb-backup/using-encryption-and-compression-tools-with-mariadb-backup) primarily discusses using external tools like `openssl` to encrypt a backup stream, `mariadb-backup` is also fully compatible with MariaDB's internal data-at-rest encryption (TDE) for InnoDB and Aria tables.
...
| **New Tables**     | If you enable encryption *during* a long-running backup, `mariadb-backup` handles it via the Redo Log. | Ensure the key used for the new table is the same one available to the backup tool.                                     |
