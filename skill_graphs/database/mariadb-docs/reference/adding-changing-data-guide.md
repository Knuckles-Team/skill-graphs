...
* [`LOAD DATA INFILE`](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/load-data-into-tables-or-index/load-data-infile): For importing data from text files.
* [`mariadb-import` utility](/docs/server/clients-and-utilities/backup-restore-and-import-clients/mariadb-import): A command-line tool that uses `LOAD DATA INFILE`. These are covered in "[Bulk Data Importing Guide](/docs/server/mariadb-quickstart-guides/mariadb-importing-data-guide)").
