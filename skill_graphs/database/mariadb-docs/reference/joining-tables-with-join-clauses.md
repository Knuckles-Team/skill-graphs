...
> Starting with MariaDB 12.3, multiple independent `LEFT JOIN` operations can be reordered by the optimizer for better performance if the [reorder\_outer\_joins](/docs/server/ha-and-performance/optimization-and-tuning/query-optimizations/reorder_outer_joins) flag is enabled in [optimizer\_switch](/docs/server/ha-and-performance/optimization-and-tuning/query-optimizations/optimizer-switch)*.* It is also recommended to set `optimizer_prune_level=0` to prevent heuristic pruning from eliminating optimal join orders.
...
* [More Advanced Joins](/docs/server/mariadb-quickstart-guides/more-advanced-joins)
* [JOIN Syntax](/docs/server/reference/sql-statements/data-manipulation/selecting-data/joins/join-syntax)
* [Comma vs JOIN](/docs/server/reference/sql-statements/data-manipulation/selecting-data/joins/comma-vs-join)
* [Joins, Subqueries and SET](/docs/server/reference/sql-structure/joins-subqueries-set)

*The initial version of this article was copied, with permission, from* [*Introduction\_to\_Joins*](https://hashmysql.org/wiki/Introduction_to_Joins) *on 2012-10-05.*
