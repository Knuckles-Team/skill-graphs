...
To add data to a table in MariaDB, you will need to use the [INSERT](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/insert) statement. Its basic, minimal syntax is the command `INSERT` followed by the table name and then the keyword `VALUES` with a comma separated list of values contained in parentheses:
...
In this SQL statement the columns in which data is to be inserted into are listed, then the complete `SELECT` statement follows with the appropriate `WHERE` clause to determine if an employee is on the softball team. Since we're executing this statement from the new database and since the table employees is in a separate database called company, we have to specify it as you see here. By the way, [INSERT...SELECT](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/insert-select) statements cannot be performed on the same table.

#### Replacement Data

When you're adding massive amounts of data to a table that has a key field, as mentioned earlier, you can use the `IGNORE` flag to prevent duplicates from being inserted, but still allow unique rows to be entered. However, there may be times when you actually want to replace the rows with the same key fields with the new ones. In such a situation, instead of using `INSERT` you can use a [REPLACE](/docs/server/reference/sql-statements/data-manipulation/changing-deleting-data/replace) statement:
...
If you want to change the data contained in existing records, but only for certain columns, then you would need to use an [UPDATE](/docs/server/reference/sql-statements/data-manipulation/changing-deleting-data/update) statement. The syntax for UPDATE is a little bit different from the syntax shown before for [INSERT](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/insert) and [REPLACE](/docs/server/reference/sql-statements/data-manipulation/changing-deleting-data/replace) statements:
...
For rows where the day of the week is Sunday, the [DATE\_ADD()](/docs/server/reference/sql-functions/date-time-functions/date_add) function will take the value of `col_date` before it's updated and add one day to it. MariaDB will then take this sum and set col\_date to it.

There are a couple more twists that you can now do with the `UPDATE` statement: if you want to update the rows in a specific order, you can add an [ORDER BY](/docs/server/reference/sql-statements/data-manipulation/selecting-data/select#order-by) clause. You can also limit the number of rows that are updated with a [LIMIT](/docs/server/reference/sql-statements/data-manipulation/selecting-data/select#limit) clause. Below is an example of both of these clauses:
...
Because of the `IGNORE` flag, errors will not be generated, duplicates won't be inserted or replaced, but the rest are added. Because of the [ON DUPLICATE KEY](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/insert-on-duplicate-key-update), the column status of the original row are set to old when there are duplicate entry attempts. The rest are inserted and their status set to new.

#### Conclusion

As you can see from some of these SQL statements, MariaDB offers you quite a few ways to add and to change data. In addition to these methods, there are also some bulk methods of adding and changing data in a table. You could use the [LOAD DATA INFILE](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/load-data-into-tables-or-index/load-data-infile) statement and the [mariadb-dump](/docs/server/clients-and-utilities/backup-restore-and-import-clients/mariadb-dump) command-line utility. These methods are covered in another article on [Importing Data into MariaDB](/docs/server/mariadb-quickstart-guides/mariadb-importing-data-guide).
