...
Complete Configuring MariaDB for Remote Client Access Guide guide for MariaDB. Complete resource with setup instructions, configuration, usage examples, and.
...
* See [Configuring MariaDB with my.cnf](/docs/server/ha-and-performance/optimization-and-tuning/system-variables/sample-mycnf-files) for comprehensive details.
* **Common Locations:**
...
4. **Save and Restart:** Save the configuration file and restart the MariaDB server service.
   * See [Starting and Stopping MariaDB](/docs/server/server-management/starting-and-stopping-mariadb/starting-and-stopping-mariadb-automatically) for instructions.
...
3. **Grant Privileges:** Use the `GRANT` statement to allow a user to connect from a remote host or a range of hosts.
   ...
   * For creating less privileged users or more granular permissions, see the [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant) documentation.
...
* **Security:** Opening MariaDB to remote connections, especially to the internet, increases security risks. Always use strong passwords, grant minimal necessary privileges, and restrict host access as much as possible. Consider using TLS/SSL for encrypted connections (see [Secure Connections Overview](/docs/server/security/encryption/data-in-transit-encryption/secure-connections-overview)).
