...
While the examples focus on the `mariadb` command-line client, the concepts apply to other clients like graphical interfaces or backup utilities (e.g., `mariadb-dump`). If you are completely new to MariaDB, refer to [A MariaDB Primer](/docs/server/mariadb-quickstart-guides/mariadb-usage-guide) first.
...
These options control the use of TLS (Transport Layer Security) for secure connections. For comprehensive details, see [Secure Connections Overview](/docs/server/security/encryption/data-in-transit-encryption/secure-connections-overview) and [TLS System Variables](/docs/server/security/encryption/data-in-transit-encryption/ssltls-system-variables).
