...
To modify an existing table, use the `ALTER TABLE` statement (see [ALTER TABLE documentation](/docs/server/reference/sql-statements/data-definition/alter/alter-table)). To delete a table and all its data (irreversibly), use `DROP TABLE table_name;` (see [DROP TABLE documentation](/docs/server/server-usage/tables/drop-table)).
...
Use the `INSERT` statement (see [INSERT documentation](/docs/server/reference/sql-statements/data-manipulation/inserting-loading-data/insert)) to add new rows to a table.
...
Since `author_id` in the `authors` table is `AUTO_INCREMENT` (see [AUTO\_INCREMENT documentation](/docs/server/reference/data-types/auto_increment)), its value is assigned automatically. If not all columns are being supplied with data, the column names must be listed, followed by their corresponding values in the `VALUES` clause.
...
To limit the number of rows returned (e.g., to 5) using `LIMIT` (see [LIMIT documentation](/docs/server/reference/sql-statements/data-manipulation/selecting-data/limit)):

```sql
SELECT title FROM books LIMIT 5;
```

To retrieve data from multiple tables, use a `JOIN` (see [JOIN documentation](/docs/server/reference/sql-statements/data-manipulation/selecting-data/joins/join-syntax)). This example lists book titles and author last names by joining `books` and `authors` on their common `author_id` column:
...
To modify existing data, use the `UPDATE` statement (see [UPDATE documentation](/docs/server/reference/sql-statements/data-manipulation/changing-deleting-data/update)). Always use a `WHERE` clause to specify which rows to update.
...
To remove rows from a table, use the `DELETE` statement (see [DELETE documentation](/docs/server/reference/sql-statements/data-manipulation/changing-deleting-data/delete)). Use `WHERE` to specify which rows to delete.
