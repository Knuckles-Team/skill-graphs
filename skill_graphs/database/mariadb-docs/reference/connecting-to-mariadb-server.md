...
This article covers connecting to MariaDB and the basic connection parameters. If you are completely new to MariaDB, take a look at [A MariaDB Primer](/docs/server/server-usage/basics/mariadb-usage-guide-1) first.

In order to connect to the MariaDB server, the client software must provide the correct connection parameters. The client software will most often be the [mariadb client](/docs/server/clients-and-utilities/mariadb-client), used for entering statements from the command line, but the same concepts apply to any client, such as a [graphical client](/docs/server/clients-and-utilities/graphical-and-enhanced-clients), a client to run backups such as [mariadb-dump](/docs/server/clients-and-utilities/backup-restore-and-import-clients/mariadb-dump), etc. The rest of this article assumes that the mariadb command line client is used.
...
Connect to the MariaDB server on the given host. The default host is `localhost`. By default, MariaDB does not permit remote logins - see [Configuring MariaDB for Remote Client Access](/docs/server/mariadb-quickstart-guides/mariadb-remote-connection-guide).
...
A brief listing is provided below. See [Secure Connections Overview](/docs/server/security/encryption/data-in-transit-encryption/secure-connections-overview) and [TLS System Variables](/docs/server/security/encryption/data-in-transit-encryption/ssltls-system-variables) for more detail.
...
The MariaDB user name to use when connecting to the server. The default is either your Unix login name, or `ODBC` on Windows. See the [GRANT](/docs/server/reference/sql-statements/account-management-sql-statements/grant) command for details on creating MariaDB user accounts.
...
* [A MariaDB Primer](/docs/server/server-usage/basics/mariadb-usage-guide-1)
* [mariadb client](/docs/server/clients-and-utilities/mariadb-client)
* [Clients and Utilities](/docs/server/clients-and-utilities)
* [Configuring MariaDB for Remote Client Access](/docs/server/mariadb-quickstart-guides/mariadb-remote-connection-guide)
* [--skip-grant-tables](/docs/server/server-management/starting-and-stopping-mariadb/mariadbd-options) allows you to start MariaDB without `GRANT`. This is useful if you lost your root password.
