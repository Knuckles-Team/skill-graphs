...
Reference of files generated during backup. This page explains the purpose of metadata files like xtrabackup\_checkpoints created by the tool.
...
This file also stores the value of the [gtid\_current\_pos](/docs/server/ha-and-performance/standard-replication/gtid#gtid_current_pos) system variable that correspond to the backup, like this:

```bash
mariadb-bin.000096 568 0-1-2
```

The values in this file are only guaranteed to be consistent with the backup if the [--no-lock](/docs/server/server-usage/backup-and-restore/mariadb-backup/mariadb-backup-options#no-lock) option was **not** provided when the backup was taken.
{% endtab %}
...
The file contains the values of the [wsrep\_local\_state\_uuid](/docs/galera-cluster/reference/galera-cluster-status-variables#wsrep_local_state_uuid) and [wsrep\_last\_committed](/docs/galera-cluster/reference/galera-cluster-status-variables#wsrep_last_committed) status variables.
