...
The kernel version.
Note that while the above files tend to be easily readable text files, they can sometimes be formatted in a way that is not easily digestible. There are many commands that do little more than read the above files and format them for easier understanding. For example, the **free** program reads `/proc/meminfo` converts the amounts given in bytes to kilobytes (and adds a little more information, as well).
* * *
