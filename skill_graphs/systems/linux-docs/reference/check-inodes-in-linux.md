[Skip to content](https://www.tecmint.com/check-inodes-in-linux/#content "Skip to content")
...
Menu
  ...
  * [Pro Courses](https://www.tecmint.com/check-inodes-in-linux/ "Linux Online Courses")
...
[](https://www.tecmint.com/check-inodes-in-linux/)
On Linux and other Unix-like operating systems, an **inode** stores information that describes a file or directory (also a file – [because everything is a file in Unix](https://www.tecmint.com/explanation-of-everything-is-a-file-and-types-of-files-in-linux/)) except its name and content or its actual data. Therefore, each file is indexed by an inode which is metadata about the file.
...
To get the number of inodes of files in a directory, for example, the **root** directory, open a terminal window and run the following [ls command](https://www.tecmint.com/tag/linux-ls-command/), where the `-l` option means long listing format, `-a` means all files and `-i` mean to print the index number of each file.
```
$ ls -lai /

```
![List Inodes in a Directory](https://www.tecmint.com/wp-content/uploads/2020/01/list-inodes-in-a-directory.png)List Inodes in a Directory
To get the total number of inodes in the root directory, run the following [du command](https://www.tecmint.com/check-linux-disk-usage-of-files-and-directories/).
```
$ sudo du --inode /

```
![List Total Number of Inodes of Root Partition](https://www.tecmint.com/wp-content/uploads/2020/01/get-total-number-of-inodes-in-root-partition.png)List Total Number of Inodes of Root Partition
To list statistics about inode usage (amount available, amount used and amount free and use percentage) in the root partition, use the [df commands](https://www.tecmint.com/how-to-check-disk-space-in-linux/) as follows (the `-h` flag allows for showing information in a human-readable format).
```
$ sudo df -ih/

```
![List Inode Usage](https://www.tecmint.com/wp-content/uploads/2020/01/show-stats-about-inode-usage.png)List Inode Usage
For a detailed inode definition, read the Linux Information Project article:
...
**Was this article helpful?** Please **[add a comment](https://www.tecmint.com/check-inodes-in-linux/#respond)** or
## Related Posts
...
### 5 Comments
[Leave a Reply](https://www.tecmint.com/check-inodes-in-linux/#reply-title)
  1. **sudo df -i** / NOT **sudo df -ih/**
[Reply](https://www.tecmint.com/check-inodes-in-linux/#comment-1411814)
  2. How to increase inode if it full and their is enough hard disk space.
[Reply](https://www.tecmint.com/check-inodes-in-linux/#comment-1365930)
     * @Arjun,
Check this article: [How to Increase Disk Inode Number in Linux](https://www.tecmint.com/increase-disk-inode-number-in-linux/ "How to Increase Disk Inode Number in Linux").
[Reply](https://www.tecmint.com/check-inodes-in-linux/#comment-1365984)
  3. [~] martin@www% (108) sudo du –inode /
du: unrecognized option ‘–inode’
Try `du –help’ for more information.
[Reply](https://www.tecmint.com/check-inodes-in-linux/#comment-1310703)
     * @Martin
The correct option is `--inode` not `-node`. Check the man page.
[Reply](https://www.tecmint.com/check-inodes-in-linux/#comment-1310832)


### Got Something to Say? Join the Discussion... [Cancel reply](https://www.tecmint.com/check-inodes-in-linux/#respond)
...
Δ
[ ](https://www.tecmint.com/check-inodes-in-linux/ "Scroll back to top")
Search for:
