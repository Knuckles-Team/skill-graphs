[ Skip to content ](https://ai.pydantic.dev/api/format_prompt/#pydantic_aiformat_prompt)
...
[ ![logo](https://ai.pydantic.dev/img/logo-white.svg) ](https://ai.pydantic.dev/ "Pydantic AI") Pydantic AI
  ...
  * API Reference
    * pydantic_ai
      ...
      * pydantic_ai.format_prompt  [ pydantic_ai.format_prompt  ](https://ai.pydantic.dev/api/format_prompt/)
        * [ format_prompt  ](https://ai.pydantic.dev/api/format_prompt/#pydantic_ai.format_prompt)
        * [ format_as_xml  ](https://ai.pydantic.dev/api/format_prompt/#pydantic_ai.format_prompt.format_as_xml)
...
`root_tag` |  |  Outer tag to wrap the XML in, use `None` to omit the outer tag. |  `None`
`item_tag` |  |  Tag to use for each item in an iterable (e.g. list), this is overridden by the class name for dataclasses and Pydantic models. |  `'item'`
`none_str` |  |  String to use for `None` values. |  `'null'`
`indent` |  |  Indentation string to use for pretty printing. |  `'  '`
`include_field_info` |  |  Whether to include attributes like Pydantic `Field` attributes and dataclasses `field()` `metadata` as XML attributes. In both cases the allowed `Field` attributes and `field()` metadata keys are `title` and `description`. If a field is repeated in the data (e.g. in a list) by setting `once` the attributes are included only in the first occurrence of an XML element relative to the same field. |  `False`
Returns:
Type | Description
---|---
