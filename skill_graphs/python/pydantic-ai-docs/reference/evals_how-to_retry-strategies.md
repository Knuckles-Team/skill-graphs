...
Pass a retry configuration to `evaluate()` or `evaluate_sync()` using
```
...
Retry configurations use [`RetryConfig`](https://ai.pydantic.dev/api/retries/#pydantic_ai.retries.RetryConfig "RetryConfig"):
```
