...
```
before: [[RetryCallState], None | [None]]
...
```
after: [[RetryCallState], None | [None]]
...
`validate_response` |  `Response], ` |  Optional callable that takes a Response and can raise an exception to be handled by the controller if the response should trigger a retry. Common use case is to raise exceptions for certain HTTP status codes. If None, no response validation is performed. |  `None`
Example
```
...
Name | Type | Description | Default
---|---|---|---
`fallback_strategy` |  `RetryCallState], ` |  Wait strategy to use when no Retry-After header is present or parsing fails. Defaults to exponential backoff with max 60s. |  `None`
`max_wait` |  |  Maximum time to wait in seconds, regardless of header value. Defaults to 300 (5 minutes). |  `300`
Returns:
Type | Description
---|---
`RetryCallState], ` |  A wait function that can be used with tenacity retry decorators.
Example
```
from httpx import AsyncClient, HTTPStatusError
from tenacity import retry_if_exception_type, stop_after_attempt
