...
Groq | ✅ | Limited parameter support. To use web search capabilities with Groq, you need to use the
OpenRouter | ✅ | Web search via `search_context_size`. Uses native search for supported providers (OpenAI, Anthropic, Perplexity, xAI), Exa for others.
...
Provider | Supported | Notes
---|---|---
OpenAI Responses | ✅ | Full feature support. Only supported by models newer than `gpt-5.2`. Metadata about the generated image, like the [`BuiltinToolReturnPart`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.BuiltinToolReturnPart "BuiltinToolReturnPart
  ...
  ") that's available via [`ModelResponse.builtin_tool_calls`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.ModelResponse.builtin_tool_calls "builtin_tool_calls
      ...
      property
  ").
Google | ✅ | Limited parameter support. Only supported by `gemini-3-pro-image-preview` and `gemini-3-pro-image-preview`. These models do not support [function tools](https://ai.pydantic.dev/tools/) and will always have the option of generating images, even if this built-in tool is not explicitly specified.
Anthropic | ❌ |
xAI | ❌ |
Groq | ❌ |
Bedrock | ❌ |
...
_(This example is complete, it can be run "as is")_
Image generation with Google `ImageGenerationTool` built-in tool to be explicitly specified:
...
Provider | Supported | Notes
---|---|---
Anthropic | ✅ | Full feature support. Uses Anthropic's
...
Provider | Supported | Notes
---|---|---
Anthropic | ✅ | Requires a tool named `memory` to be defined that implements
Google | ❌ |
OpenAI | ❌ |
Groq | ❌ |
Bedrock | ❌ |
...
HuggingFace | ❌ |
### Usage
The Anthropic SDK provides an abstract
...
Provider | Supported | Notes
---|---|---
OpenAI Responses | ✅ | Full feature support. `x-openai-connector:<connector_id>` URL.
Anthropic | ✅ | Full feature support
xAI | ✅ | Full feature support
...
For OpenAI Responses, you can use a `x-openai-connector:` URL:
_(This example is complete, it can be run "as is")_
...
```
  ...
  1. OpenAI's Google Calendar connector requires an
...
Provider | Supported | Notes
---|---|---
OpenAI Responses | ✅ | Full feature support. Requires files to be uploaded to vector stores via the [`BuiltinToolReturnPart`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.BuiltinToolReturnPart "BuiltinToolReturnPart
  ...
  "), enable the [`OpenAIResponsesModelSettings.openai_include_file_search_results`](https://ai.pydantic.dev/api/models/openai/#pydantic_ai.models.openai.OpenAIResponsesModelSettings.openai_include_file_search_results "openai_include_file_search_results
      ...
      instance-attribute
  ") [model setting](https://ai.pydantic.dev/agent/#model-run-settings).
Google (Gemini) | ✅ | Requires files to be uploaded via the [output tools](https://ai.pydantic.dev/output/#tool-output)) at the same time is not supported; to use structured output, use [`PromptedOutput`](https://ai.pydantic.dev/output/#prompted-output) instead.
| Google (Vertex AI) | ❌
...
### Usage
#### OpenAI Responses
With OpenAI, you need to first `FileSearchTool`.
