...
On this page
  ...
  * [SSE Polling for Long-Running Operations](https://gofastmcp.com/deployment/http#sse-polling-for-long-running-operations)
  * [Custom Storage Backends](https://gofastmcp.com/deployment/http#custom-storage-backends)
  ...
  * [Horizontal Scaling](https://gofastmcp.com/deployment/http#horizontal-scaling)
  * [Understanding Sessions](https://gofastmcp.com/deployment/http#understanding-sessions)
  * [Without Stateless Mode](https://gofastmcp.com/deployment/http#without-stateless-mode)
  * [Enabling Stateless Mode](https://gofastmcp.com/deployment/http#enabling-stateless-mode)
  ...
  * [Reverse Proxy (nginx)](https://gofastmcp.com/deployment/http#reverse-proxy-nginx)
  * [Running FastMCP as a Linux Service](https://gofastmcp.com/deployment/http#running-fastmcp-as-a-linux-service)
  * [nginx Configuration](https://gofastmcp.com/deployment/http#nginx-configuration)
  * [Key Considerations](https://gofastmcp.com/deployment/http#key-considerations)
  * [Mounting Under a Path Prefix](https://gofastmcp.com/deployment/http#mounting-under-a-path-prefix)
...
FastMCP supports multiple authentication methods to secure your remote server. See the [Authentication Overview](https://gofastmcp.com/servers/auth/authentication) for complete configuration options including Bearer tokens, JWT, and OAuth. If you’re mounting an authenticated server under a path prefix, see [Mounting Authenticated Servers](https://gofastmcp.com/deployment/http#mounting-authenticated-servers) below for important routing considerations.
###
...
###
[​](https://gofastmcp.com/deployment/http#sse-polling-for-long-running-operations)
...
####
[​](https://gofastmcp.com/deployment/http#custom-storage-backends)
...
FastAPI Integration
For FastAPI-specific integration patterns including both mounting MCP servers into FastAPI apps and generating MCP servers from FastAPI apps, see the [FastAPI Integration guide](https://gofastmcp.com/integrations/fastapi). Here’s a quick example showing how to add MCP to an existing FastAPI application:
Copy
```
...
###
[​](https://gofastmcp.com/deployment/http#horizontal-scaling)
Horizontal Scaling
...
####
[​](https://gofastmcp.com/deployment/http#understanding-sessions)
Understanding Sessions
By default, FastMCP’s Streamable HTTP transport maintains server-side sessions. Sessions enable stateful MCP features like [elicitation](https://gofastmcp.com/servers/elicitation) and [sampling](https://gofastmcp.com/servers/sampling), where the server needs to maintain context across multiple requests from the same client. This works perfectly for single-instance deployments. However, sessions are stored in memory on each server instance, which creates challenges when scaling horizontally.
####
[​](https://gofastmcp.com/deployment/http#without-stateless-mode)
...
####
[​](https://gofastmcp.com/deployment/http#enabling-stateless-mode)
...
##
[​](https://gofastmcp.com/deployment/http#reverse-proxy-nginx)
Reverse Proxy (nginx)
...
###
[​](https://gofastmcp.com/deployment/http#running-fastmcp-as-a-linux-service)
...
###
[​](https://gofastmcp.com/deployment/http#nginx-configuration)
nginx Configuration
...
###
[​](https://gofastmcp.com/deployment/http#key-considerations)
Key Considerations
When deploying FastMCP behind a reverse proxy, keep these points in mind:
  ...
  * **Increase timeouts** : The default nginx `proxy_read_timeout` is 60 seconds. Long-running MCP tools will cause the connection to drop. Set timeouts to at least 300 seconds, or higher if your tools run longer. For tools that may exceed any timeout, use [SSE Polling](https://gofastmcp.com/deployment/http#sse-polling-for-long-running-operations) to gracefully handle proxy disconnections.
...
###
[​](https://gofastmcp.com/deployment/http#mounting-under-a-path-prefix)
...
[ Running Your Server Previous ](https://gofastmcp.com/deployment/running-server)[ Prefect Horizon Next ](https://gofastmcp.com/deployment/prefect-horizon)
Ctrl+I
