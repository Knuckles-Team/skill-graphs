[Skip to main content](https://gofastmcp.com/v2/servers/auth/remote-oauth#content-area)
...
On this page
  * [DCR-Enabled Providers](https://gofastmcp.com/v2/servers/auth/remote-oauth#dcr-enabled-providers)
  * [The Remote OAuth Challenge](https://gofastmcp.com/v2/servers/auth/remote-oauth#the-remote-oauth-challenge)
  * [MCP Authentication Discovery](https://gofastmcp.com/v2/servers/auth/remote-oauth#mcp-authentication-discovery)
  * [FastMCP Remote Authentication](https://gofastmcp.com/v2/servers/auth/remote-oauth#fastmcp-remote-authentication)
  * [RemoteAuthProvider](https://gofastmcp.com/v2/servers/auth/remote-oauth#remoteauthprovider)
  * [Basic Implementation](https://gofastmcp.com/v2/servers/auth/remote-oauth#basic-implementation)
  * [Custom Endpoints](https://gofastmcp.com/v2/servers/auth/remote-oauth#custom-endpoints)
  * [WorkOS AuthKit Integration](https://gofastmcp.com/v2/servers/auth/remote-oauth#workos-authkit-integration)
  * [Client Redirect URI Security](https://gofastmcp.com/v2/servers/auth/remote-oauth#client-redirect-uri-security)
  * [Implementation Considerations](https://gofastmcp.com/v2/servers/auth/remote-oauth#implementation-considerations)
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#dcr-enabled-providers)
...
**FastMCP Class** | `RemoteAuthProvider` | [`OAuthProxy`](https://gofastmcp.com/v2/servers/auth/oauth-proxy)
If your provider doesn’t support DCR (most traditional OAuth providers), you’ll need to use [`OAuth Proxy`](https://gofastmcp.com/v2/servers/auth/oauth-proxy) instead, which bridges the gap between MCP’s DCR expectations and fixed OAuth credentials.
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#the-remote-oauth-challenge)
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#mcp-authentication-discovery)
MCP Authentication Discovery
MCP authentication discovery relies on well-known endpoints that clients can examine to understand your authentication requirements. Your server becomes a bridge between MCP clients and your chosen identity provider. The core discovery endpoint is `/.well-known/oauth-protected-resource`, which tells clients that your server requires OAuth authentication and identifies the authorization servers you trust. This endpoint contains static metadata that points clients to your identity provider without requiring any dynamic lookups.
Identity ProviderFastMCP ServerClientIdentity ProviderFastMCP ServerClientClient goes directly to the IdP1. GET /.well-known/oauth-protected-resource2. "Use https://my-idp.com for auth"3. Authenticate & get token via DCR4. Access token5. MCP request with Bearer token6. Verify token signature7. MCP response
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#fastmcp-remote-authentication)
FastMCP Remote Authentication
`2.11.1` FastMCP provides `RemoteAuthProvider` to handle the complexities of remote OAuth integration. This class combines token validation capabilities with the OAuth discovery metadata that MCP clients require.
###
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#remoteauthprovider)
RemoteAuthProvider
`RemoteAuthProvider` works by composing a [`TokenVerifier`](https://gofastmcp.com/v2/servers/auth/token-verification) with authorization server information. A `TokenVerifier` is another FastMCP authentication class that focuses solely on token validation - signature verification, expiration checking, and claim extraction. The `RemoteAuthProvider` takes that token validation capability and adds the OAuth discovery endpoints that enable MCP clients to automatically find and authenticate with your identity provider. This composition pattern means you can use any token validation strategy while maintaining consistent OAuth discovery behavior:
...
###
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#custom-endpoints)
Custom Endpoints
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#workos-authkit-integration)
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#client-redirect-uri-security)
...
##
[​](https://gofastmcp.com/v2/servers/auth/remote-oauth#implementation-considerations)
...
[ Token Verification Previous ](https://gofastmcp.com/v2/servers/auth/token-verification)[ OAuth Proxy Next ](https://gofastmcp.com/v2/servers/auth/oauth-proxy)
Ctrl+I
