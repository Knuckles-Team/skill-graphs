...
When generating an MCP server from FastAPI, FastMCP uses OpenAPIProvider (v3.0.0+) under the hood to source tools from your FastAPI app’s OpenAPI spec. See [Providers](https://gofastmcp.com/servers/providers/overview) to understand how FastMCP sources components.
...
If you’re mounting an authenticated MCP server under a path prefix, see [Mounting Authenticated Servers](https://gofastmcp.com/deployment/http#mounting-authenticated-servers) for important OAuth routing considerations.
###
...
[ WorkOS 🤝 FastMCP Previous ](https://gofastmcp.com/integrations/workos)[ OpenAPI 🤝 FastMCP Next ](https://gofastmcp.com/integrations/openapi)
Ctrl+I
