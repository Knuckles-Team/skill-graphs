...
##  In this article
  1. [Description](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#description)
  2. [Use cases](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#use-cases)
  3. [Permissions](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#permissions)
  4. [Best practices](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#best-practices)
  5. [Security note: side-channel attacks](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#security-note-side-channel-attacks)
  6. [Cross-feature compatibility](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#cross-feature-compatibility)
  7. [Examples](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#examples)
  8. [Related content](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#related-content)
...
![Decorative graphic of row level security.](https://learn.microsoft.com/en-us/sql/relational-databases/security/media/row-level-security/row-level-security-graphic.png?view=sql-server-ver17)
...
Implement RLS by using the [CREATE SECURITY POLICY](https://learn.microsoft.com/en-us/sql/t-sql/statements/create-security-policy-transact-sql?view=sql-server-ver17) Transact-SQL statement, and predicates created as [Inline table-valued functions](https://learn.microsoft.com/en-us/sql/relational-databases/user-defined-functions/create-user-defined-functions-database-engine?view=sql-server-ver17).
Row-level security was first introduced to SQL Server 2016 (13.x).
This article is focused on SQL Server and Azure SQL platforms. For Microsoft Fabric, see [Row-level security in Microsoft Fabric](https://learn.microsoft.com/en-us/fabric/data-warehouse/row-level-security).
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#description)
## Description
...
Block predicates have the following behavior:
  * Block predicates for `UPDATE` are split into separate operations for `BEFORE` and `AFTER`. You can't, for example, block users from updating a row to have a value higher than the current one. If this kind of logic is required, you must use triggers with the [DELETED and INSERTED](https://learn.microsoft.com/en-us/sql/relational-databases/triggers/use-the-inserted-and-deleted-tables?view=sql-server-ver17) intermediate tables to reference the old and new values together.
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#use-cases)
## Use cases
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#permissions)
## Permissions
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#best-practices)
## Best practices
...
Avoid predicate logic that depends on session-specific [SET options](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-statements-transact-sql?view=sql-server-ver17): While unlikely to be used in practical applications, predicate functions whose logic depends on certain session-specific `SET` options can leak information if users are able to execute arbitrary queries. For example, a predicate function that implicitly converts a string to **datetime** could filter different rows based on the `SET DATEFORMAT` option for the current session. In general, predicate functions should abide by the following rules:
  * Predicate functions shouldn't implicitly convert character strings to **date** , **smalldatetime** , **datetime** , **datetime2** , or **datetimeoffset** , or vice versa, because these conversions are affected by the [SET DATEFORMAT (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-dateformat-transact-sql?view=sql-server-ver17) and [SET LANGUAGE (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-language-transact-sql?view=sql-server-ver17) options. Instead, use the `CONVERT` function and explicitly specify the style parameter.
  * Predicate functions shouldn't rely on the value of the first day of the week, because this value is affected by the [SET DATEFIRST (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-datefirst-transact-sql?view=sql-server-ver17) option.
  * Predicate functions shouldn't rely on arithmetic or aggregation expressions returning `NULL` if they error (such as overflow or divide-by-zero), because this behavior is affected by the [SET ANSI_WARNINGS (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-ansi-warnings-transact-sql?view=sql-server-ver17), [SET NUMERIC_ROUNDABORT (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-numeric-roundabort-transact-sql?view=sql-server-ver17), and [SET ARITHABORT (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-arithabort-transact-sql?view=sql-server-ver17) options.
  * Predicate functions shouldn't compare concatenated strings with `NULL`, because this behavior is affected by the [SET CONCAT_NULL_YIELDS_NULL (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/set-concat-null-yields-null-transact-sql?view=sql-server-ver17) option.


[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#security-note-side-channel-attacks)
## Security note: side-channel attacks
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#malicious-security-policy-manager)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#carefully-crafted-queries)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#cross-feature-compatibility)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#examples)
## Examples
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#a-scenario-for-users-who-authenticate-to-the-database)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#b-scenarios-for-using-row-level-security-on-an-azure-synapse-external-table)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#prerequisites)
### Prerequisites
  1. You must have a dedicated SQL pool. See [Create a dedicated SQL pool](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql-data-warehouse/create-data-warehouse-portal)
  2. The server hosting your dedicated SQL pool must be registered with Microsoft Entra ID ([formerly Azure Active Directory](https://learn.microsoft.com/en-us/entra/fundamentals/new-name)) and you must have an Azure storage account with `Storage Blog Data Contributor` permissions. Follow the steps to [Use virtual network service endpoints and rules for servers in Azure SQL Database](https://learn.microsoft.com/en-us/azure/azure-sql/database/vnet-service-endpoint-rule-overview#steps).
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#c-scenario-for-users-who-connect-to-the-database-through-a-middle-tier-application)
...
This example shows how a middle-tier application can implement connection filtering, where application users (or tenants) share the same SQL Server user (the application). The application sets the current application user ID in [SESSION_CONTEXT](https://learn.microsoft.com/en-us/sql/t-sql/functions/session-context-transact-sql?view=sql-server-ver17) after connecting to the database, and then security policies transparently filter rows that shouldn't be visible to this ID, and also block the user from inserting rows for the wrong user ID. No other app changes are necessary.
Create a table to hold data.
SQL
Copy
```
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#d-scenario-for-using-a-lookup-table-for-the-security-predicate)
...
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#e-row-level-security-scenario-in-microsoft-fabric)
...
After applying the security policy and creating the function, the users `Sales1@contoso.com` and `Sales2@contoso.com` will only be able to see their own data in the `sales.Orders` table, where the column `SalesRep` equals their own user name returned by the built-in function [USER_NAME](https://learn.microsoft.com/en-us/sql/t-sql/functions/user-name-transact-sql?view=sql-server-ver17). The Fabric user `manager@contoso.com` is able to see all data in the `sales.Orders` table.
[](https://learn.microsoft.com/en-us/sql/relational-databases/security/row-level-security?view=sql-server-ver17#related-content)
## Related content
  * [CREATE SECURITY POLICY (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/create-security-policy-transact-sql?view=sql-server-ver17)
  * [ALTER SECURITY POLICY (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/alter-security-policy-transact-sql?view=sql-server-ver17)
  * [DROP SECURITY POLICY (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/statements/drop-security-policy-transact-sql?view=sql-server-ver17)
  ...
  * [SESSION_CONTEXT (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/functions/session-context-transact-sql?view=sql-server-ver17)
  * [sp_set_session_context (Transact-SQL)](https://learn.microsoft.com/en-us/sql/relational-databases/system-stored-procedures/sp-set-session-context-transact-sql?view=sql-server-ver17)
  * [sys.security_policies (Transact-SQL)](https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-security-policies-transact-sql?view=sql-server-ver17)
  * [sys.security_predicates (Transact-SQL)](https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-security-predicates-transact-sql?view=sql-server-ver17)
...
##  Additional resources
  * [ CREATE SECURITY POLICY (Transact-SQL) - SQL Server ](https://learn.microsoft.com/en-us/sql/t-sql/statements/create-security-policy-transact-sql?source=recommendations)
CREATE SECURITY POLICY a security policy for use with row-level security.
  * [ ALTER SECURITY POLICY (Transact-SQL) - SQL Server ](https://learn.microsoft.com/en-us/sql/t-sql/statements/alter-security-policy-transact-sql?source=recommendations)
ALTER SECURITY POLICY alters a security policy for row-level security.
  * [ Dynamic Data Masking - SQL Server ](https://learn.microsoft.com/en-us/sql/relational-databases/security/dynamic-data-masking?source=recommendations)
Learn about dynamic data masking, which limits sensitive data exposure by masking it to nonprivileged users. It can greatly simplify security in SQL Server.
  * [ sys.security_policies (Transact-SQL) - SQL Server ](https://learn.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-security-policies-transact-sql?source=recommendations)
sys.security_policies (Transact-SQL)


Module
[ Secure Data Access in Power BI - Training ](https://learn.microsoft.com/en-us/training/modules/row-level-security-power-bi/?source=recommendations)
...
[English (United States)](https://learn.microsoft.com/en-us/locale?target=https%3A%2F%2Flearn.microsoft.com%2Fen-us%2Fsql%2Frelational-databases%2Fsecurity%2Frow-level-security%3Fview%3Dsql-server-ver17)
Theme
  * Light
  * Dark
  * High contrast
