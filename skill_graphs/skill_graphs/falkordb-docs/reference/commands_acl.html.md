[Skip to main content](https://docs.falkordb.com/commands/graph.profile.html#main-content) Link Menu Expand (external link) Document Search Copy Copied
...
#  [](https://docs.falkordb.com/commands/graph.profile.html#graphprofile) GRAPH.PROFILE
...
It is important to note that this blends elements of [GRAPH.QUERY](https://docs.falkordb.com/commands/graph.query) and [GRAPH.EXPLAIN](https://docs.falkordb.com/commands/graph.explain). It is not a dry run and will perform all graph modifications expected of the query, but will not output results produced by a `RETURN` clause or query statistics.
