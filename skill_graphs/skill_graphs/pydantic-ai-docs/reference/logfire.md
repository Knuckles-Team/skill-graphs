...
Since Logfire is built on [below](https://ai.pydantic.dev/logfire/#using-opentelemetry).
### Debugging
...
To observe raw HTTP requests made to model providers, you can use Logfire's [HTTPX instrumentation](https://logfire.pydantic.dev/docs/integrations/http-clients/httpx/) since all provider SDKs (except for [Bedrock](https://ai.pydantic.dev/models/bedrock/)) use the
...
## Using OpenTelemetry
Pydantic AI's instrumentation uses
This means you can debug and monitor Pydantic AI with any OpenTelemetry backend.
Pydantic AI follows the ![😉](https://cdn.jsdelivr.net/gh/jdecked/twemoji@16.0.1/assets/svg/1f609.svg), you should be able to use any OTel service with GenAI support.
...
```
  ...
  1. Set the `OTEL_EXPORTER_OTLP_ENDPOINT` environment variable to the URL of your OpenTelemetry backend. If you're using a backend that requires authentication, you may need to set `export OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318`.
...
Custom namespace
The `gen_ai.aggregated_usage.*` namespace is a custom extension not part of the
```
from pydantic_ai import Agent
...
Pydantic AI follows the `version` parameter of [`InstrumentationSettings`](https://ai.pydantic.dev/api/models/instrumented/#pydantic_ai.models.instrumented.InstrumentationSettings "InstrumentationSettings
...
Based on `event_mode='logs'` to emit events as OpenTelemetry log-based events:
...
#### Version 4
Builds on version 3 with improved multimodal content handling to better align with the
