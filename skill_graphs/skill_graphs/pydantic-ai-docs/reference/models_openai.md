    ...
    * Models & Providers
      ...
      * OpenAI  [ OpenAI  ](https://ai.pydantic.dev/models/openai/)
        ...
        * [ OpenAI Responses API  ](https://ai.pydantic.dev/models/openai/#openai-responses-api)
          * [ Built-in tools  ](https://ai.pydantic.dev/models/openai/#built-in-tools)
            * [ Referencing earlier responses  ](https://ai.pydantic.dev/models/openai/#referencing-earlier-responses)
              * [ Automatically referencing earlier responses  ](https://ai.pydantic.dev/models/openai/#automatically-referencing-earlier-responses)
        * [ OpenAI-compatible Models  ](https://ai.pydantic.dev/models/openai/#openai-compatible-models)
          ...
          * [ MoonshotAI  ](https://ai.pydantic.dev/models/openai/#moonshotai)
...
## Configuration
To use `OpenAIChatModel` with the OpenAI API, go to
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/models/openai/#__tabbed_2_1)[Directly to Provider API](https://ai.pydantic.dev/models/openai/#__tabbed_2_2)
...
`OpenAIProvider` also accepts a custom `AsyncOpenAI` client via the `openai_client` parameter, so you can customise the `organization`, `project`, `base_url` etc. as defined in the
...
You could also use the `AsyncAzureOpenAI` is a subclass of `AsyncOpenAI`.
```
...
Pydantic AI also supports OpenAI's
You can use [`OpenAIResponsesModel`](https://ai.pydantic.dev/api/models/openai/#pydantic_ai.models.openai.OpenAIResponsesModel "OpenAIResponsesModel



      dataclass
  ") by name:
[With Pydantic AI Gateway](https://ai.pydantic.dev/models/openai/#__tabbed_3_1)[Directly to Provider API](https://ai.pydantic.dev/models/openai/#__tabbed_3_2)
[Learn about Gateway](https://ai.pydantic.dev/gateway)```
from pydantic_ai import Agent
...
Computer use can be enabled by passing an `openai_builtin_tools` setting on [`OpenAIResponsesModelSettings`](https://ai.pydantic.dev/api/models/openai/#pydantic_ai.models.openai.OpenAIResponsesModelSettings "OpenAIResponsesModelSettings"). It doesn't currently generate [`BuiltinToolCallPart`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.BuiltinToolCallPart "BuiltinToolCallPart
...
The Responses API supports referencing earlier model responses in a new request using a `previous_response_id` parameter, to ensure the full `openai_previous_response_id` field in [`OpenAIResponsesModelSettings`](https://ai.pydantic.dev/api/models/openai/#pydantic_ai.models.openai.OpenAIResponsesModelSettings "OpenAIResponsesModelSettings").
```
from pydantic_ai import Agent
from pydantic_ai.models.openai import OpenAIResponsesModel, OpenAIResponsesModelSettings
...
Many providers and models are compatible with the OpenAI API, and can be used with `OpenAIChatModel` in Pydantic AI. Before getting started, check the [installation and configuration](https://ai.pydantic.dev/models/openai/#install) instructions above.
...
To use Qwen models via `ALIBABA_API_KEY` (or `DASHSCOPE_API_KEY`) environment variable and use [`AlibabaProvider`](https://ai.pydantic.dev/api/providers/#pydantic_ai.providers.alibaba.AlibabaProvider) by name:
```
from pydantic_ai import Agent
...
### Ollama
Pydantic AI supports both self-hosted
...
ollama_model = OpenAIChatModel(
    ...
    provider=OllamaProvider(base_url='http://localhost:11434/v1'),  [](https://ai.pydantic.dev/models/openai/#__code_24_annotation_1)
)
...
### Azure AI Foundry
To use `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_API_KEY`, and `OPENAI_API_VERSION` environment variables and use [`AzureProvider`](https://ai.pydantic.dev/api/providers/#pydantic_ai.providers.azure.AzureProvider "AzureProvider") by name:
```
from pydantic_ai import Agent
...
### LiteLLM
To use `LiteLLMProvider`, you can pass `api_base` and `api_key`. The value of these configs will depend on your setup. For example, if you are using OpenAI models, then you need to pass `https://api.openai.com/v1` as the `api_base` and your OpenAI API key as the `api_key`. If you are using a LiteLLM proxy server running on your local machine, then you need to pass `http://localhost:<port>` as the `api_base` and your LiteLLM API key (or a placeholder) as the `api_key`.
...
To use OVHcloud AI Endpoints, you need to create a new API key. To do so, go to the `Create a new API key` and copy your new key.
You can explore the
...
For a complete list of available models, see the
