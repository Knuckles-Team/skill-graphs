## Setup
For details on how to set up authentication with this model, see [model configuration for Google](https://ai.pydantic.dev/models/google/).
...
```
GoogleModelName =  | LatestGoogleModelNames[](https://ai.pydantic.dev/api/models/google/#pydantic_ai.models.google.LatestGoogleModelNames "LatestGoogleModelNames



      module-attribute
   \(pydantic_ai.models.google.LatestGoogleModelNames\)")

```

Possible Gemini model names.
Since Gemini supports a variety of date-stamped models, we explicitly list the latest models but allow any name in the type hints. See
###  GoogleModelSettings
...
```
google_safety_settings: [SafetySettingDict]
...
This is implemented from scratch rather than using a dedicated SDK, good API documentation is available
