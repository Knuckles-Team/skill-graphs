...
Pydantic AI comes with three ways to connect to MCP servers:
  * [`MCPServerStreamableHTTP`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerStreamableHTTP "MCPServerStreamableHTTP") which connects to an MCP server using the
  * [`MCPServerSSE`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerSSE "MCPServerSSE") which connects to an MCP server using the
  * [`MCPServerStdio`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerStdio "MCPServerStdio") which runs the server as a subprocess and connects to it using the
...
[`MCPServerStreamableHTTP`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerStreamableHTTP "MCPServerStreamableHTTP") connects over HTTP using the
Note
...
### SSE Client
[`MCPServerSSE`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerSSE "MCPServerSSE") connects over HTTP using the
Note
...
_(This example is complete, it can be run "as is" — you'll need to add`asyncio.run(main())` to run `main`)_
### MCP "stdio" Server
MCP also offers `stdin` and `stdout`. In this case, you'd use the [`MCPServerStdio`](https://ai.pydantic.dev/api/mcp/#pydantic_ai.mcp.MCPServerStdio "MCPServerStdio") class.
...
How to access the metadata is MCP server SDK specific. For example with the
mcp_server.py```
from typing import Any
...
## Resources
MCP servers can provide _not_ be exposed to the LLM automatically (unless a tool returns a `ResourceLink` or `EmbeddedResource`).
...
When connecting to an MCP server, you can optionally specify an
...
Elicitation introduces a new protocol message type called `ErrorData` message.
...
MCP elicitation supports string, number, boolean, and enum types with flat object structures only. These limitations ensure reliable cross-client compatibility. See
### Security
MCP Elicitation requires careful handling - servers must not request sensitive information, and clients must implement user approval controls with clear explanations. See
