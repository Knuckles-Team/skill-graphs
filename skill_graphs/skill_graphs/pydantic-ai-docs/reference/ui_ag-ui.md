...
[ ![logo](https://ai.pydantic.dev/img/logo-white.svg) ](https://ai.pydantic.dev/ "Pydantic AI") Pydantic AI
  ...
  * Integrations
    ...
    * UI Event Streams
      ...
      * AG-UI  [ AG-UI  ](https://ai.pydantic.dev/ui/ag-ui/)
        * [ Installation  ](https://ai.pydantic.dev/ui/ag-ui/#installation)
        * [ Usage  ](https://ai.pydantic.dev/ui/ag-ui/#usage)
          * [ Handle run input and output directly  ](https://ai.pydantic.dev/ui/ag-ui/#handle-run-input-and-output-directly)
          * [ Handle a Starlette request  ](https://ai.pydantic.dev/ui/ag-ui/#handle-a-starlette-request)
          * [ Stand-alone ASGI app  ](https://ai.pydantic.dev/ui/ag-ui/#stand-alone-asgi-app)
        * [ Design  ](https://ai.pydantic.dev/ui/ag-ui/#design)
        * [ Features  ](https://ai.pydantic.dev/ui/ag-ui/#features)
          * [ State management  ](https://ai.pydantic.dev/ui/ag-ui/#state-management)
          * [ Tools  ](https://ai.pydantic.dev/ui/ag-ui/#tools)
          * [ Events  ](https://ai.pydantic.dev/ui/ag-ui/#events)
        * [ Examples  ](https://ai.pydantic.dev/ui/ag-ui/#examples)
...
The
Note
The AG-UI integration was originally built by the team at
## Installation
The only dependencies are:
You can install Pydantic AI with the `ag-ui` extra to ensure you have all the required AG-UI dependencies:
[pip](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_1_1)[uv](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_1_2)
```
...
[pip](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_2_1)[uv](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_2_2)
```
pip install uvicorn
...
There are three ways to run a Pydantic AI agent based on AG-UI run input with streamed AG-UI events as output, from most to least flexible. If you're using a Starlette-based web framework like FastAPI, you'll typically want to use the second method.
  1. The [`AGUIAdapter.run_stream()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.run_stream "run_stream") method, when called on an [`AGUIAdapter`](https://ai.pydantic.dev/api/ui/ag_ui/#pydantic_ai.ui.ag_ui.AGUIAdapter "AGUIAdapter
      ...
      dataclass
  ") instantiated with an agent and an AG-UI [`Agent.iter()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.iter "iter
  ...
  3. [`AGUIApp`](https://ai.pydantic.dev/api/ui/ag_ui/#pydantic_ai.ui.ag_ui.app.AGUIApp "AGUIApp") represents an ASGI application that handles every AG-UI request by running the agent. It also takes optional [`Agent.iter()`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.Agent.iter "iter
      ...
      async
  ") arguments including `deps`, but these will be the same for each request, with the exception of the AG-UI state that's injected as described under [state management](https://ai.pydantic.dev/ui/ag-ui/#state-management). This ASGI app can be
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_3_1)[Directly to Provider API](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_3_2)
...
[](https://ai.pydantic.dev/ui/ag-ui/#__code_5_annotation_1)
    except ValidationError as e:
        return Response(
            content=json.dumps(e.json()),
            media_type='application/json',
...
[](https://ai.pydantic.dev/ui/ag-ui/#__code_5_annotation_2)

    sse_event_stream = adapter.encode_stream(event_stream)
    return StreamingResponse(sse_event_stream, media_type=accept) #

[](https://ai.pydantic.dev/ui/ag-ui/#__code_5_annotation_3)
...
```
  ...
  1. [`AGUIAdapter.build_run_input()`](https://ai.pydantic.dev/api/ui/ag_ui/#pydantic_ai.ui.ag_ui.AGUIAdapter.build_run_input "build_run_input
      ...
      classmethod
  ") takes the request body as bytes and returns an AG-UI [`AGUIAdapter.from_request()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.from_request "from_request
  ...
  2. [`AGUIAdapter.run_stream()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.run_stream "run_stream") runs the agent and returns a stream of AG-UI events. It supports the same optional arguments as [`Agent.run_stream_events()`](https://ai.pydantic.dev/agent/#running-agents), including `deps`. You can also use [`AGUIAdapter.run_stream_native()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.run_stream_native "run_stream_native") to run the agent and return a stream of Pydantic AI events instead, which can then be transformed into AG-UI events using [`AGUIAdapter.transform_stream()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.transform_stream "transform_stream").
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_4_1)[Directly to Provider API](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_4_2)
...
[](https://ai.pydantic.dev/ui/ag-ui/#__code_8_annotation_1)
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_5_1)[Directly to Provider API](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_5_2)
...
In the example below we have document state which is shared between the UI and server using the [`StateDeps`](https://ai.pydantic.dev/api/ag_ui/#pydantic_ai.ag_ui.StateDeps "StateDeps
      ...
      dataclass
  ") [dependencies type](https://ai.pydantic.dev/dependencies/) that can be used to automatically validate state contained in `BaseModel` specified as a generic parameter.
Custom dependencies type with AG-UI state
If you want to use your own dependencies type to hold AG-UI state as well as other things, it needs to implements the [`StateHandler`](https://ai.pydantic.dev/api/ag_ui/#pydantic_ai.ag_ui.StateHandler "StateHandler") protocol, meaning it needs to be a `state` field. This lets Pydantic AI ensure that state is properly isolated between requests by building a new dependencies object each time.
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_6_1)[Directly to Provider API](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_6_2)
...
### Events
Pydantic AI tools can send [`ToolReturn`](https://ai.pydantic.dev/tools-advanced/#advanced-tool-returns) object with a `metadata`, which allows for custom events and state updates.
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_7_1)[Directly to Provider API](https://ai.pydantic.dev/ui/ag-ui/#__tabbed_7_2)
...
## Examples
For more examples of how to use [`AGUIApp`](https://ai.pydantic.dev/api/ui/ag_ui/#pydantic_ai.ui.ag_ui.app.AGUIApp "AGUIApp") see
