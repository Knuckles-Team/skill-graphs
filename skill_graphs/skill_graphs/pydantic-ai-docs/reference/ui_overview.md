[ Skip to content ](https://ai.pydantic.dev/ui/overview/#ui-event-streams)
...
[ ![logo](https://ai.pydantic.dev/img/logo-white.svg) ](https://ai.pydantic.dev/ "Pydantic AI") Pydantic AI
  ...
  * Integrations
    ...
    * UI Event Streams
      * Overview  [ Overview  ](https://ai.pydantic.dev/ui/overview/)
        * [ Usage  ](https://ai.pydantic.dev/ui/overview/#usage)
          * [ Usage with Starlette/FastAPI  ](https://ai.pydantic.dev/ui/overview/#usage-with-starlettefastapi)
          * [ Advanced Usage  ](https://ai.pydantic.dev/ui/overview/#advanced-usage)
...
# UI Event Streams
If you're building a chat app or other interactive frontend for an AI agent, your backend will need to receive agent run input (like a chat message or complete [message history](https://ai.pydantic.dev/message-history/)) from the frontend, and will need to stream the [agent's events](https://ai.pydantic.dev/agent/#streaming-all-events) (like text, thinking, and tool calls) to the frontend so that the user knows what's happening in real time.
While your frontend could use Pydantic AI's [`ModelRequest`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.ModelRequest "ModelRequest



      dataclass
  ") and [`AgentStreamEvent`](https://ai.pydantic.dev/api/messages/#pydantic_ai.messages.AgentStreamEvent "AgentStreamEvent
...
These integrations are implemented as subclasses of the abstract [`UIAdapter`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter "UIAdapter
...
## Usage
The protocol-specific [`UIAdapter`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter "UIAdapter



      dataclass
  ") subclass (i.e. [`AGUIAdapter`](https://ai.pydantic.dev/api/ui/ag_ui/#pydantic_ai.ui.ag_ui.AGUIAdapter "AGUIAdapter



      dataclass
  ") or [`VercelAIAdapter`](https://ai.pydantic.dev/api/ui/vercel_ai/#pydantic_ai.ui.vercel_ai.VercelAIAdapter "VercelAIAdapter



      dataclass
  ")) is responsible for transforming agent run input received from the frontend into arguments for [`Agent.run_stream_events()`](https://ai.pydantic.dev/agent/#running-agents), running the agent, and then transforming Pydantic AI events into protocol-specific events. The event stream transformation is handled by a protocol-specific [`UIEventStream`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIEventStream "UIEventStream



      dataclass
  ") subclass, but you typically won't use this directly.
If you're using a Starlette-based web framework like FastAPI, you can use the [`UIAdapter.dispatch_request()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.dispatch_request "dispatch_request
...
Besides the request, [`UIAdapter.dispatch_request()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.dispatch_request "dispatch_request
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/overview/#__tabbed_1_1)[Directly to Provider API](https://ai.pydantic.dev/ui/overview/#__tabbed_1_2)
...
If you're using a web framework not based on Starlette (e.g. Django or Flask) or need fine-grained control over the input or output, you can create a `UIAdapter` instance and directly use its methods, which can be chained to accomplish the same thing as the `UIAdapter.dispatch_request()` class method shown above:
  1. The [`UIAdapter.build_run_input()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.build_run_input "build_run_input



      abstractmethod
      classmethod
  ") class method takes the request body as bytes and returns a protocol-specific run input object, which you can then pass to the [`UIAdapter()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter "UIAdapter



      dataclass
  ") constructor along with the agent.
     * You can also use the [`UIAdapter.from_request()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.from_request "from_request
  ...
  2. The [`UIAdapter.run_stream()`](https://ai.pydantic.dev/api/ui/base/#pydantic_ai.ui.UIAdapter.run_stream "run_stream") method runs the agent and returns a stream of protocol-specific events. It supports the same optional arguments as [`Agent.run_stream_events()`](https://ai.pydantic.dev/agent/#running-agents) and an optional `on_complete` callback function that receives the completed [`AgentRunResult`](https://ai.pydantic.dev/api/agent/#pydantic_ai.agent.AgentRunResult "AgentRunResult
...
[With Pydantic AI Gateway](https://ai.pydantic.dev/ui/overview/#__tabbed_2_1)[Directly to Provider API](https://ai.pydantic.dev/ui/overview/#__tabbed_2_2)
