## Copying model instances[¶](https://docs.djangoproject.com/en/5.0/topics/db/queries/#copying-model-instances "Link to this heading")
Although there is no built-in method for copying model instances, it is possible to easily create new instance with all fields’ values copied. In the simplest case, you can set `pk` to `None` and [`_state.adding`](https://docs.djangoproject.com/en/5.0/ref/models/instances/#django.db.models.Model._state "django.db.models.Model._state") to `True`. Using our blog example:
```
