# Migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#module-django.db.migrations "Link to this heading")
Migrations are Django’s way of propagating changes you make to your models (adding a field, deleting a model, etc.) into your database schema. They’re designed to be mostly automatic, but you’ll need to know when to make migrations, when to run them, and the common problems you might run into.
## The Commands[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#the-commands "Link to this heading")
There are several commands which you will use to interact with migrations and Django’s handling of database schema:
  * [`migrate`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-migrate), which is responsible for applying and unapplying migrations.
  * [`makemigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-makemigrations), which is responsible for creating new migrations based on the changes you have made to your models.
  * [`sqlmigrate`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-sqlmigrate), which displays the SQL statements for a migration.
  * [`showmigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-showmigrations), which lists a project’s migrations and their status.
...
Note
It is possible to override the name of the package which contains the migrations on a per-app basis by modifying the [`MIGRATION_MODULES`](https://docs.djangoproject.com/en/5.0/ref/settings/#std-setting-MIGRATION_MODULES) setting.
...
## Backend Support[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#backend-support "Link to this heading")
Migrations are supported on all backends that Django ships with, as well as any third-party backends if they have programmed in support for schema alteration (done via the [SchemaEditor](https://docs.djangoproject.com/en/5.0/ref/schema-editor/) class).
...
### PostgreSQL[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#postgresql "Link to this heading")
PostgreSQL is the most capable of all the databases here in terms of schema support.
### MySQL[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#mysql "Link to this heading")
...
### SQLite[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#sqlite "Link to this heading")
...
## Workflow[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#workflow "Link to this heading")
Django can create migrations for you. Make changes to your models - say, add a field and remove a model - and then run [`makemigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-makemigrations):
```
...
If you want to give the migration(s) a meaningful name instead of a generated one, you can use the [`makemigrations --name`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#cmdoption-makemigrations-name) option:
```
$ python manage.py makemigrations --name changed_my_model your_app_label

```

### Version control[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#version-control "Link to this heading")
...
It’s also possible to execute parts of the migration inside a transaction using [`atomic()`](https://docs.djangoproject.com/en/5.0/topics/db/transactions/#django.db.transaction.atomic "django.db.transaction.atomic") or by passing `atomic=True` to [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython"). See [Non-atomic migrations](https://docs.djangoproject.com/en/5.0/howto/writing-migrations/#non-atomic-migrations) for more details.
## Dependencies[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#dependencies "Link to this heading")
...
### Swappable dependencies[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#swappable-dependencies "Link to this heading")

django.db.migrations.swappable_dependency(_value_)[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#django.db.migrations.django.db.migrations.swappable_dependency "Link to this definition")
...
## Migration files[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#migration-files "Link to this heading")
...
### Custom fields[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#custom-fields "Link to this heading")
...
### Model managers[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#model-managers "Link to this heading")
You can optionally serialize managers into migrations and have them available in [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython") operations. This is done by defining a `use_in_migrations` attribute on the manager class:
```
...
If you are using the [`from_queryset()`](https://docs.djangoproject.com/en/5.0/topics/db/managers/#django.db.models.from_queryset "django.db.models.from_queryset") function to dynamically generate a manager class, you need to inherit from the generated class to make it importable:
```
...
Please refer to the notes about [Historical models](https://docs.djangoproject.com/en/5.0/topics/migrations/#historical-models) in migrations to see the implications that come along.
### Initial migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#initial-migrations "Link to this heading")

Migration.initial[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#django.db.migrations.Migration.initial "Link to this definition")
...
When the [`migrate --fake-initial`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#cmdoption-migrate-fake-initial) option is used, these initial migrations are treated specially. For an initial migration that creates one or more tables (`CreateModel` operation), Django checks that all of those tables already exist in the database and fake-applies the migration if so. Similarly, for an initial migration that adds one or more fields (`AddField` operation), Django checks that all of the respective columns already exist in the database and fake-applies the migration if so. Without `--fake-initial`, initial migrations are treated no differently from any other migration.
### History consistency[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#history-consistency "Link to this heading")
As previously discussed, you may need to linearize migrations manually when two development branches are joined. While editing migration dependencies, you can inadvertently create an inconsistent history state where a migration has been applied but some of its dependencies haven’t. This is a strong indication that the dependencies are incorrect, so Django will refuse to run migrations or make new migrations until it’s fixed. When using multiple databases, you can use the [`allow_migrate()`](https://docs.djangoproject.com/en/5.0/topics/db/multi-db/#allow_migrate "allow_migrate") method of [database routers](https://docs.djangoproject.com/en/5.0/topics/db/multi-db/#topics-db-multi-db-routing) to control which databases [`makemigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-makemigrations) checks for consistent history.
## Adding migrations to apps[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#adding-migrations-to-apps "Link to this heading")
New apps come preconfigured to accept migrations, and so you can add migrations by running [`makemigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-makemigrations) once you’ve made some changes.
...
This will make a new initial migration for your app. Now, run `python manage.py migrate --fake-initial`, and Django will detect that you have an initial migration _and_ that the tables it wants to create already exist, and will mark the migration as already applied. (Without the [`migrate --fake-initial`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#cmdoption-migrate-fake-initial) flag, the command would error out because the tables it wants to create already exist.)
...
## Reversing migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#reversing-migrations "Link to this heading")
Migrations can be reversed with [`migrate`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-migrate) by passing the number of the previous migration. For example, to reverse migration `books.0003`:
/ 
```
...
To remove old references, you can [squash migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#migration-squashing) or, if there aren’t many references, copy them into the migration files.
## Considerations when removing model fields[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#considerations-when-removing-model-fields "Link to this heading")
...
To help with this situation, Django provides some model field attributes to assist with model field deprecation using the [system checks framework](https://docs.djangoproject.com/en/5.0/topics/checks/).
...
## Data Migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#data-migrations "Link to this heading")
...
Now, all you need to do is create a new function and have [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython") use it. [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython") expects a callable as its argument which takes two arguments - the first is an [app registry](https://docs.djangoproject.com/en/5.0/ref/applications/) that has the historical versions of all your models loaded into it to match where in your history the migration sits, and the second is a [SchemaEditor](https://docs.djangoproject.com/en/5.0/ref/schema-editor/), which you can use to manually effect database schema changes (but beware, doing this can confuse the migration autodetector!)
...
You can pass a second callable to [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython") to run whatever logic you want executed when migrating backwards. If this callable is omitted, migrating backwards will raise an exception.
### Accessing models from other apps[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#accessing-models-from-other-apps "Link to this heading")
...
### More advanced migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#more-advanced-migrations "Link to this heading")
If you’re interested in the more advanced migration operations, or want to be able to write your own, see the [migration operations reference](https://docs.djangoproject.com/en/5.0/ref/migration-operations/) and the “how-to” on [writing migrations](https://docs.djangoproject.com/en/5.0/howto/writing-migrations/).
## Squashing migrations[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#squashing-migrations "Link to this heading")
...
Django does this by taking all of your existing migrations, extracting their `Operation`s and putting them all in sequence, and then running an optimizer over them to try and reduce the length of the list - for example, it knows that [`CreateModel`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.CreateModel "django.db.migrations.operations.CreateModel") and [`DeleteModel`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.DeleteModel "django.db.migrations.operations.DeleteModel") cancel each other out, and it knows that [`AddField`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.AddField "django.db.migrations.operations.AddField") can be rolled into [`CreateModel`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.CreateModel "django.db.migrations.operations.CreateModel").
Once the operation sequence has been reduced as much as possible - the amount possible depends on how closely intertwined your models are and if you have any [`RunSQL`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunSQL "django.db.migrations.operations.RunSQL") or [`RunPython`](https://docs.djangoproject.com/en/5.0/ref/migration-operations/#django.db.migrations.operations.RunPython "django.db.migrations.operations.RunPython") operations (which can’t be optimized through unless they are marked as `elidable`) - Django will then write it back out into a new set of migration files.
...
The command that backs all this is [`squashmigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-squashmigrations) - pass it the app label and migration name you want to squash up to, and it’ll get to work:
```
...
Use the [`squashmigrations --squashed-name`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#cmdoption-squashmigrations-squashed-name) option if you want to set the name of the squashed migration rather than use an autogenerated one.
...
If it is likely that you may reuse the name of a deleted migration in the future, you should remove references to it from Django’s migrations table with the [`migrate --prune`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#cmdoption-migrate-prune) option.
## Serializing values[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#serializing-values "Link to this heading")
...
Django can serialize the following:
  ...
  * Anything with a custom `deconstruct()` method ([see below](https://docs.djangoproject.com/en/5.0/topics/migrations/#custom-deconstruct-method))


Changed in Django 4.2:
Serialization support for `enum.Flag` was added.
Changed in Django 5.0:
Serialization support for functions decorated with
...
### Custom serializers[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#custom-serializers "Link to this heading")
You can serialize other types by writing a custom serializer. For example, if Django didn’t serialize
```
...
### Adding a `deconstruct()` method[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#adding-a-deconstruct-method "Link to this heading")
...
Note
This return value is different from the `deconstruct()` method [for custom fields](https://docs.djangoproject.com/en/5.0/howto/custom-model-fields/#custom-field-deconstruct-method) which returns a tuple of four items.
...
## Supporting multiple Django versions[¶](https://docs.djangoproject.com/en/5.0/topics/migrations/#supporting-multiple-django-versions "Link to this heading")
If you are the maintainer of a third-party app with models, you may need to ship migrations that support multiple Django versions. In this case, you should always run [`makemigrations`](https://docs.djangoproject.com/en/5.0/ref/django-admin/#django-admin-makemigrations) **with the lowest Django version you wish to support**.
...
[The Migrations Operations Reference](https://docs.djangoproject.com/en/5.0/ref/migration-operations/)

Covers the schema operations API, special operations, and writing your own operations.

[The Writing Migrations “how-to”](https://docs.djangoproject.com/en/5.0/howto/writing-migrations/)
...
[](https://docs.djangoproject.com/en/5.0/topics/migrations/#top)
## Additional Information
### Support Django!
![Support Django!](https://static.djangoproject.com/img/fundraising-heart.cd6bb84ffd33.svg)
  * [ Boone & Davis, Attorneys at Law donated to the Django Software Foundation to support Django development. Donate today! ](https://www.djangoproject.com/fundraising/)


### Contents
  * [Migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/)
    * [The Commands](https://docs.djangoproject.com/en/5.0/topics/migrations/#the-commands)
    * [Backend Support](https://docs.djangoproject.com/en/5.0/topics/migrations/#backend-support)
      * [PostgreSQL](https://docs.djangoproject.com/en/5.0/topics/migrations/#postgresql)
      * [MySQL](https://docs.djangoproject.com/en/5.0/topics/migrations/#mysql)
      * [SQLite](https://docs.djangoproject.com/en/5.0/topics/migrations/#sqlite)
    * [Workflow](https://docs.djangoproject.com/en/5.0/topics/migrations/#workflow)
      * [Version control](https://docs.djangoproject.com/en/5.0/topics/migrations/#version-control)
    * [Transactions](https://docs.djangoproject.com/en/5.0/topics/migrations/#transactions)
    * [Dependencies](https://docs.djangoproject.com/en/5.0/topics/migrations/#dependencies)
      * [Swappable dependencies](https://docs.djangoproject.com/en/5.0/topics/migrations/#swappable-dependencies)
    * [Migration files](https://docs.djangoproject.com/en/5.0/topics/migrations/#migration-files)
      * [Custom fields](https://docs.djangoproject.com/en/5.0/topics/migrations/#custom-fields)
      * [Model managers](https://docs.djangoproject.com/en/5.0/topics/migrations/#model-managers)
      * [Initial migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#initial-migrations)
      * [History consistency](https://docs.djangoproject.com/en/5.0/topics/migrations/#history-consistency)
    * [Adding migrations to apps](https://docs.djangoproject.com/en/5.0/topics/migrations/#adding-migrations-to-apps)
    * [Reversing migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#reversing-migrations)
    * [Historical models](https://docs.djangoproject.com/en/5.0/topics/migrations/#historical-models)
    * [Considerations when removing model fields](https://docs.djangoproject.com/en/5.0/topics/migrations/#considerations-when-removing-model-fields)
    * [Data Migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#data-migrations)
      * [Accessing models from other apps](https://docs.djangoproject.com/en/5.0/topics/migrations/#accessing-models-from-other-apps)
      * [More advanced migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#more-advanced-migrations)
    * [Squashing migrations](https://docs.djangoproject.com/en/5.0/topics/migrations/#squashing-migrations)
    * [Serializing values](https://docs.djangoproject.com/en/5.0/topics/migrations/#serializing-values)
      * [Custom serializers](https://docs.djangoproject.com/en/5.0/topics/migrations/#custom-serializers)
      * [Adding a `deconstruct()` method](https://docs.djangoproject.com/en/5.0/topics/migrations/#adding-a-deconstruct-method)
    * [Supporting multiple Django versions](https://docs.djangoproject.com/en/5.0/topics/migrations/#supporting-multiple-django-versions)


### Browse
  * Prev: [Using mixins with class-based views](https://docs.djangoproject.com/en/5.0/topics/class-based-views/mixins/)
  * Next: [Managing files](https://docs.djangoproject.com/en/5.0/topics/files/)
