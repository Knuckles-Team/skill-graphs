[Skip to main content](https://gofastmcp.com/v2/servers/composition#content-area)
...
On this page
  * [Why Compose Servers?](https://gofastmcp.com/v2/servers/composition#why-compose-servers)
  * [Importing vs Mounting](https://gofastmcp.com/v2/servers/composition#importing-vs-mounting)
  * [Proxy Servers](https://gofastmcp.com/v2/servers/composition#proxy-servers)
  * [Importing (Static Composition)](https://gofastmcp.com/v2/servers/composition#importing-static-composition)
  * [How Importing Works](https://gofastmcp.com/v2/servers/composition#how-importing-works)
  * [Importing Without Prefixes](https://gofastmcp.com/v2/servers/composition#importing-without-prefixes)
  * [Conflict Resolution](https://gofastmcp.com/v2/servers/composition#conflict-resolution)
  * [Mounting (Live Linking)](https://gofastmcp.com/v2/servers/composition#mounting-live-linking)
  * [How Mounting Works](https://gofastmcp.com/v2/servers/composition#how-mounting-works)
  * [Performance Considerations](https://gofastmcp.com/v2/servers/composition#performance-considerations)
  * [Mounting Without Prefixes](https://gofastmcp.com/v2/servers/composition#mounting-without-prefixes)
  * [Direct vs. Proxy Mounting](https://gofastmcp.com/v2/servers/composition#direct-vs-proxy-mounting)
  * [Interaction with Proxy Servers](https://gofastmcp.com/v2/servers/composition#interaction-with-proxy-servers)
  * [Tag Filtering with Composition](https://gofastmcp.com/v2/servers/composition#tag-filtering-with-composition)
  * [How Recursive Filtering Works](https://gofastmcp.com/v2/servers/composition#how-recursive-filtering-works)
...
Copy page
`2.2.0` As your MCP applications grow, you might want to organize your tools, resources, and prompts into logical modules or reuse existing server components. FastMCP supports composition through two methods:
  * **`import_server`**: For a one-time copy of components with prefixing (static composition).
  * **`mount`**: For creating a live link where the main server delegates requests to the subserver (dynamic composition).


##
[​](https://gofastmcp.com/v2/servers/composition#why-compose-servers)
Why Compose Servers?
...
###
[​](https://gofastmcp.com/v2/servers/composition#importing-vs-mounting)
...
###
[​](https://gofastmcp.com/v2/servers/composition#proxy-servers)
Proxy Servers
FastMCP supports [MCP proxying](https://gofastmcp.com/v2/servers/proxy), which allows you to mirror a local or remote server in a local FastMCP instance. Proxies are fully compatible with both importing and mounting. `2.4.0` You can also create proxies from configuration dictionaries that follow the MCPConfig schema, which is useful for quickly connecting to one or more remote servers. See the [Proxy Servers documentation](https://gofastmcp.com/v2/servers/proxy#configuration-based-proxies) for details on configuration-based proxying. Note that MCPConfig follows an emerging standard and its format may evolve over time. Prefixing rules for tools, prompts, resources, and templates are identical across importing, mounting, and proxies. When prefixes are used, resource URIs are prefixed using path format (since 2.4.0): `resource://prefix/path/to/resource`.
##
[​](https://gofastmcp.com/v2/servers/composition#importing-static-composition)
...
###
[​](https://gofastmcp.com/v2/servers/composition#how-importing-works)
How Importing Works
...
####
[​](https://gofastmcp.com/v2/servers/composition#importing-without-prefixes)
Importing Without Prefixes
`2.9.0` You can also import servers without specifying a prefix, which copies components using their original names:
Copy
```
...
####
[​](https://gofastmcp.com/v2/servers/composition#conflict-resolution)
Conflict Resolution
`2.9.0` When importing multiple servers with the same prefix, or no prefix, components from the **most recently imported** server take precedence.
##
[​](https://gofastmcp.com/v2/servers/composition#mounting-live-linking)
...
###
[​](https://gofastmcp.com/v2/servers/composition#how-mounting-works)
How Mounting Works
...
####
[​](https://gofastmcp.com/v2/servers/composition#performance-considerations)
Performance Considerations
Due to the “live link”, operations like `list_tools()` on the parent server will be impacted by the speed of the slowest mounted server. In particular, HTTP-based mounted servers can introduce significant latency (300-400ms vs 1-2ms for local tools), and this slowdown affects the whole server, not just interactions with the HTTP-proxied tools. If performance is important, importing tools via [`import_server()`](https://gofastmcp.com/v2/servers/composition#importing-static-composition) may be a more appropriate solution as it copies components once at startup rather than delegating requests at runtime.
####
[​](https://gofastmcp.com/v2/servers/composition#mounting-without-prefixes)
Mounting Without Prefixes
`2.9.0` You can also mount servers without specifying a prefix, which makes components accessible without prefixing. This works identically to [importing without prefixes](https://gofastmcp.com/v2/servers/composition#importing-without-prefixes), including [conflict resolution](https://gofastmcp.com/v2/servers/composition#conflict-resolution).
###
[​](https://gofastmcp.com/v2/servers/composition#direct-vs-proxy-mounting)
Direct vs. Proxy Mounting
`2.2.7` FastMCP supports two mounting modes:
...
####
[​](https://gofastmcp.com/v2/servers/composition#interaction-with-proxy-servers)
...
##
[​](https://gofastmcp.com/v2/servers/composition#tag-filtering-with-composition)
Tag Filtering with Composition
`2.9.0` When using `include_tags` or `exclude_tags` on a parent server, these filters apply **recursively** to all components, including those from mounted or imported servers. This allows you to control which components are exposed at the parent level, regardless of how your application is composed.
Copy
```
import asyncio
from fastmcp import FastMCP, Client
...
###
[​](https://gofastmcp.com/v2/servers/composition#how-recursive-filtering-works)
...
[ Prompts Previous ](https://gofastmcp.com/v2/servers/prompts)[ MCP Context Next ](https://gofastmcp.com/v2/servers/context)
Ctrl+I
