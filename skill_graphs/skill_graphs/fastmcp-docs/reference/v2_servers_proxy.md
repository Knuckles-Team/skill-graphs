[Skip to main content](https://gofastmcp.com/v2/servers/proxy#content-area)
...
On this page
  * [What is Proxying?](https://gofastmcp.com/v2/servers/proxy#what-is-proxying)
  * [Key Benefits](https://gofastmcp.com/v2/servers/proxy#key-benefits)
  * [Performance Considerations](https://gofastmcp.com/v2/servers/proxy#performance-considerations)
  * [Quick Start](https://gofastmcp.com/v2/servers/proxy#quick-start)
  * [Session Isolation & Concurrency](https://gofastmcp.com/v2/servers/proxy#session-isolation-%26-concurrency)
  * [Fresh Sessions](https://gofastmcp.com/v2/servers/proxy#fresh-sessions)
  * [Session Reuse with Connected Clients](https://gofastmcp.com/v2/servers/proxy#session-reuse-with-connected-clients)
  * [Transport Bridging](https://gofastmcp.com/v2/servers/proxy#transport-bridging)
  * [Advanced MCP Features](https://gofastmcp.com/v2/servers/proxy#advanced-mcp-features)
  * [Supported Features](https://gofastmcp.com/v2/servers/proxy#supported-features)
  * [Customizing Feature Support](https://gofastmcp.com/v2/servers/proxy#customizing-feature-support)
  * [Configuration-Based Proxies](https://gofastmcp.com/v2/servers/proxy#configuration-based-proxies)
  * [Multi-Server Configurations](https://gofastmcp.com/v2/servers/proxy#multi-server-configurations)
  * [Component Prefixing](https://gofastmcp.com/v2/servers/proxy#component-prefixing)
  * [Mirrored Components](https://gofastmcp.com/v2/servers/proxy#mirrored-components)
  * [FastMCPProxy Class](https://gofastmcp.com/v2/servers/proxy#fastmcpproxy-class)
  * [Direct Usage](https://gofastmcp.com/v2/servers/proxy#direct-usage)
  * [Parameters](https://gofastmcp.com/v2/servers/proxy#parameters)
  * [Explicit Session Management](https://gofastmcp.com/v2/servers/proxy#explicit-session-management)
...
Copy page
`2.0.0` FastMCP provides a powerful proxying capability that allows one FastMCP server instance to act as a frontend for another MCP server (which could be remote, running on a different transport, or even another FastMCP instance). This is achieved using the `FastMCP.as_proxy()` class method.
##
[​](https://gofastmcp.com/v2/servers/proxy#what-is-proxying)
What is Proxying?
...
###
[​](https://gofastmcp.com/v2/servers/proxy#key-benefits)
Key Benefits
`2.10.3`
...
###
[​](https://gofastmcp.com/v2/servers/proxy#performance-considerations)
Performance Considerations
When using proxy servers, especially those connecting to HTTP-based backend servers, be aware that latency can be significant. Operations like `list_tools()` may take hundreds of milliseconds compared to 1-2ms for local tools. When mounting proxy servers, this latency affects all operations on the parent server, not just interactions with the proxied tools. If low latency is a requirement for your use-case, consider using [`import_server()`](https://gofastmcp.com/v2/servers/composition#importing-static-composition) to copy tools at startup rather than proxying them at runtime.
##
[​](https://gofastmcp.com/v2/servers/proxy#quick-start)
Quick Start
`2.10.3` The recommended way to create a proxy is using `ProxyClient`, which provides full MCP feature support with automatic session isolation:
Copy
```
...
You can also pass a FastMCP [client transport](https://gofastmcp.com/v2/clients/transports) (or parameter that can be inferred to a transport) to `as_proxy()`. This will automatically create a `ProxyClient` instance for you. Finally, you can pass a regular FastMCP `Client` instance to `as_proxy()`. This will work for many use cases, but may break if advanced MCP features like sampling or elicitation are invoked by the server.
##
[​](https://gofastmcp.com/v2/servers/proxy#session-isolation-&-concurrency)
Session Isolation & Concurrency
`2.10.3` FastMCP proxies provide session isolation to ensure safe concurrent operations. The session strategy depends on how the proxy is configured:
###
[​](https://gofastmcp.com/v2/servers/proxy#fresh-sessions)
Fresh Sessions
...
###
[​](https://gofastmcp.com/v2/servers/proxy#session-reuse-with-connected-clients)
...
##
[​](https://gofastmcp.com/v2/servers/proxy#transport-bridging)
Transport Bridging
...
##
[​](https://gofastmcp.com/v2/servers/proxy#advanced-mcp-features)
Advanced MCP Features
`2.10.3` `ProxyClient` automatically forwards advanced MCP protocol features between the backend server and clients connected to the proxy, ensuring full MCP compatibility.
###
[​](https://gofastmcp.com/v2/servers/proxy#supported-features)
Supported Features
...
###
[​](https://gofastmcp.com/v2/servers/proxy#customizing-feature-support)
...
##
[​](https://gofastmcp.com/v2/servers/proxy#configuration-based-proxies)
Configuration-Based Proxies
`2.4.0` You can create a proxy directly from a configuration dictionary that follows the MCPConfig schema. This is useful for quickly setting up proxies to remote servers without manually configuring each connection detail.
Copy
```
from fastmcp import FastMCP
...
###
[​](https://gofastmcp.com/v2/servers/proxy#multi-server-configurations)
...
##
[​](https://gofastmcp.com/v2/servers/proxy#component-prefixing)
Component Prefixing
...
##
[​](https://gofastmcp.com/v2/servers/proxy#fastmcpproxy-class)
`FastMCPProxy` Class
...
###
[​](https://gofastmcp.com/v2/servers/proxy#direct-usage)
Direct Usage
Copy
```
...
###
[​](https://gofastmcp.com/v2/servers/proxy#parameters)
Parameters
...
###
[​](https://gofastmcp.com/v2/servers/proxy#explicit-session-management)
...
[ Progress Reporting Previous ](https://gofastmcp.com/v2/servers/progress)[ LLM Sampling Next ](https://gofastmcp.com/v2/servers/sampling)
Ctrl+I
