...
Next
##  _Homenaje a la Neurocirugía_ by Marta Colvin Andrade
### (1 of 12)
![A bronze statue of two crossed hands delicately holding a human brain in their fingertips.](https://i.imgur.com/Mx7dA2Y.jpg)
...
> The `[` and `]` syntax here is called `useState` always has exactly two items.
...
Instead, to enable their concise syntax, Hooks **rely on a stable call order on every render of the same component.** This works well in practice because if you follow the rule above (“only call Hooks at the top level”), Hooks will always be called in the same order. Additionally, a
Internally, React holds an array of state pairs for every component. It also maintains the current pair index, which is set to `0` before rendering. Each time you call `useState`, React gives you the next state pair and increments the index. You can read more about this mechanism in
