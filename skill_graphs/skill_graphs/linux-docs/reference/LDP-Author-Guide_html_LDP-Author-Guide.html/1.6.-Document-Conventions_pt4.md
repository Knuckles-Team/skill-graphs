...
![](https://tldp.org/LDP/LDP-Author-Guide/html/neditsuccess.jpg)
* * *
###  B.2.2.5. VIM
...
Vim has a DocBook helper script which can be easily copied into your `.vimscripts` directory and used to "auto complete" tags while writing DocBook documents. The script can be downloaded from:
...
* * *
###  B.2.2.6. XMLForm
This web-based application allows you to put in the URL for XML source, or copy and paste the XML directly into the web form. The application then breaks down your document into a series of form fields that hide the DocBook tags so that you may edit the content directly. Version 5 is available from
...
![Note](https://tldp.org/LDP/LDP-Author-Guide/images/note.gif) | **The Docbook Section**
---|---
|  There is more information about how to validate your document in the DocBook section. Check out [Section B.3](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#tools-validate) for more help with validating your document.
* * *
##  B.3.2. Validation for the Faint of Heart
Your life is already hard enough without having to install a full set of tools just to see if you validate as well. You can upload your raw XML files to a web site, then go to
![Note](https://tldp.org/LDP/LDP-Author-Guide/images/note.gif) | **External entities**
---|---
...
* * *
###  B.3.3.1. Catalogs
XML and SGML files contain most of the information you need; however, there are sometimes entities which are specific to SGML in general. To match these entities to their actual values you need to use a _catalog_. The role of a catalog is to tell your system where to find the files it is looking for. You may want to think of a catalog as a guide book (or a map) for your tools.
...
The structure of XML catalog files is not the same as SGML catalog files. The section on tailoring a catalog (see [Section B.3.4](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#making-catalogs)) will give more details about what these files actually contain.
...
```
                                                           ![\(1\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/1.gif)
-- Catalog for the Conectiva Styles --

OVERRIDE YES
                                                          ![\(2\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/2.gif)
PUBLIC "-//Conectiva SA//DTD DocBook Conectiva variant V1.0//EN"
			"/home/ldp/styles/books.dtd"

DELEGATE "-//OASIS"
        	"/home/ldp/SGML/dtds/catalog.dtd"
                                                          ![\(3\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/3.gif)
...
[![\(1\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/1.gif)](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#ex.catalog.comment)
     Comment. Comments start with "--" and follow to the end of the line.

[![\(2\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/2.gif)](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#ex.catalog.definition)
     The public type association `_"-//Conectiva SA//DTD books V1.0//EN"_` with the file `books.dtd` on the directory `/home/ldp/styles`.

[![\(3\)](https://tldp.org/LDP/LDP-Author-Guide/images/callouts/3.gif)](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#ex.catalog.eof)
...
As in the example above, to associate an identifier to a file just follow the sequence shown:
  1. Copy the identifier _PUBLIC_
...
###  B.3.5.1. nsgmls
You can use nsgmls, which is part of the jade suite (on Debian apt-get the docbook-utils package, see [Section B.4.2](https://tldp.org/LDP/LDP-Author-Guide/html/LDP-Author-Guide.html#docbook-utils)), to validate SGML or XML documents.
```

`bash$` **nsgmls `_-s_` `_HOWTO.xml_`**

```

---
If there are no issues, you'll just get your command prompt back. The `_-s_` tells nsgmls to show only the errors.
![Tip](https://tldp.org/LDP/LDP-Author-Guide/images/tip.gif) | **Function not found**
---|---
|  If you get errors about a function not being found, or something about an ISO character not having an authoritative source, you may need to point **nsgmls** to your `xml.dcl` file. For Red Hat 9, it will look like this: **nsgmls` _-s_` `/usr/share/sgml/xml.dcl` `_HOWTO.xml_`**
For more information on processing files with Jade/OpenJade please read [DocBook XML/SGML Processing Using OpenJade](http://www.tldp.org/HOWTO/DocBook-OpenJade-SGML-XML-HOWTO/index.html).
* * *
###  B.3.5.2. onsgmls
...
`bash$ `**onsgmls `_-s_` `_-wxml_` `_-wno-explicit-sgml-decl_` `_HOWTO.xml_`**
...
To prevent printing the entire document to your screen, add the `_--noout_` parameter.
```

`bash$` **xmllint `_--noout_` `_HOWTO.xml_`**
...
If you would like to check your document for any errors which are specific to your Document Type Definition, add `_--valid_`.
```

`bash$` **xmllint `_--noout_` `_--valid_` `_HOWTO.xml_`**
...
If you are a Mac OSX or Windows user, you may also want to check out tkxmllint, a GUI version of xmllint. More information is available from:
...
* * *
#  B.4. Transformations
![Warning](https://tldp.org/LDP/LDP-Author-Guide/images/warning.gif) | **TLDP will convert your document**
---|---
|  This section is about how to transform documents from DocBook to other formats. If you do not need to transform documents for your own web site, or to proof read the content, please _skip this section_. If you would like to transform your documents for proofreading purposes, please use the  Another easy-to-use package is xmlto. It is a front-end for xsltproc. It is available as a RedHat, Debian (etc) package or can be downloaded from  | ```
