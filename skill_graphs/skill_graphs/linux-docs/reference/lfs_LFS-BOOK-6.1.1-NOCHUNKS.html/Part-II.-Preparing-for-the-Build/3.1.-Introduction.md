...
Download locations may not always be accessible. If a download location has changed since this book was published, Google (
...
To create this directory, execute, as user _root_ , the following command before starting the download session:
```
`mkdir -v $LFS/sources`
```
