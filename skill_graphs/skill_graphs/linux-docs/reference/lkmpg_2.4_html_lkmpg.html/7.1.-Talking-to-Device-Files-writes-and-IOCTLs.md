...
The answer in Unix is to use a special function called `ioctl` (short for Input Output ConTroL). Every device can have its own `ioctl` commands, which can be read `ioctl`'s (to send information from a process to the kernel), write `ioctl`'s (to return information to a process), [[9]](https://tldp.org/LDP/lkmpg/2.4/html/lkmpg.html#FTN.AEN878) both or neither. The `ioctl` function is called with three parameters: the file descriptor of the appropriate device file, the ioctl number, and a parameter, which is of type long so you can use a cast to use it to pass anything. [[10]](https://tldp.org/LDP/lkmpg/2.4/html/lkmpg.html#FTN.AEN883)
