[Skip to content](https://vuejs.org/guide/quick-start.html#VPContent)
...
[简体中文 ](https://cn.vuejs.org/guide/quick-start)
[日本語 ](https://ja.vuejs.org/guide/quick-start)
[Українська ](https://ua.vuejs.org/guide/quick-start)
[Français ](https://fr.vuejs.org/guide/quick-start)
[한국어 ](https://ko.vuejs.org/guide/quick-start)
[Português ](https://pt.vuejs.org/guide/quick-start)
[বাংলা ](https://bn.vuejs.org/guide/quick-start)
[Italiano ](https://it.vuejs.org/guide/quick-start)
[فارسی ](https://fa.vuejs.org/guide/quick-start)
[Русский ](https://ru.vuejs.org/guide/quick-start)
[Čeština ](https://cs.vuejs.org/guide/quick-start)
[繁體中文 ](https://zh-hk.vuejs.org/guide/quick-start)
[Polski ](https://pl.vuejs.org/guide/quick-start)
Help Us Translate!
Appearance
Menu
On this page
...
Prerequisites
  ...
  * Install `^20.19.0 || >=22.12.0`


In this section we will introduce how to scaffold a Vue [Single Page Application](https://vuejs.org/guide/extras/ways-of-using-vue#single-page-application-spa) on your local machine. The created project will be using a build setup based on [Single-File Components](https://vuejs.org/guide/scaling-up/sfc) (SFCs).
Make sure you have an up-to-date version of `$` sign):
npmpnpmyarnbun
sh```
...
You should now have your first Vue project running! Note that the example components in the generated project are written using the [Composition API](https://vuejs.org/guide/introduction#composition-api) and `<script setup>`, rather than the [Options API](https://vuejs.org/guide/introduction#options-api). Here are some additional tips:
  * The recommended IDE setup is [IDE support section](https://vuejs.org/guide/scaling-up/tooling#ide-support).
...
We can teach the browser where to locate the `vue` import by using
html```
...
Import Maps is a relatively new browser feature. Make sure to use a browser within its
...
While it is possible to use Vue without a build system, an alternative approach to consider is using
...
To start a local HTTP server, first make sure you have `npx serve` from the command line in the same directory where your HTML file is. You can also use any other HTTP server that can serve static files with the correct MIME types.
You may have noticed that the imported component's template is inlined as a JavaScript string. If you are using VS Code, you can install the `/*html*/` comment to get syntax highlighting for them.
## Next Steps [​](https://vuejs.org/guide/quick-start.html#next-steps)
...
[Continue with the Guide The guide walks you through every aspect of the framework in full detail.](https://vuejs.org/guide/essentials/application)[Try the Tutorial For those who prefer learning things hands-on.](https://vuejs.org/tutorial/)[Check out the Examples Explore examples of core features and common UI tasks.](https://vuejs.org/examples/)
