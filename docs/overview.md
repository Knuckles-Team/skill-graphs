...
Documentation-sourced skill graphs for domain-specific expertise.
...
| **Audit Logging** | ✅ Built-in | Append-only compliance trail with 30+ action types |
| **Token Usage Analytics** | ✅ Built-in | 4-bucket tracking with budget alerting |
| **Prompt Injection Defense** | ✅ Built-in | 25+ pattern scanner + jailbreak taxonomy (CONCEPT:OS-5.1) |
| **Guardrail Engine** | ✅ Built-in | Input/output interception with block/redact/warn |
...
| **Session Concurrency** | ✅ Built-in | Enqueue/reject/interrupt/rollback |
...
```json
{
  "mcpServers": {
    "skill-graphs": {
      "command": "uv",
      "args": ["run", "--with", "skill-graphs", "skill-mcp"],
      "env": {}
    }
  }
}
...
```bash
skill-mcp --transport streamable-http --port 8001
```
